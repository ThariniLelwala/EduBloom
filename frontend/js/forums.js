...
function loadPendingTopics() {
    const pendingTopics = JSON.parse(localStorage.getItem('pendingTopics') || '[]');
    const pendingContainer = document.getElementById('pending-topics');

    if (pendingTopics.length === 0) {
        pendingContainer.innerHTML = '<p style="color: rgba(255, 255, 255, 0.6); text-align: center; padding: 20px;">No topics pending approval</p>';
        return;
    }
    ...
    pendingTopics.forEach(topic => {
        ...
        topicElement.innerHTML = `
            <div class="pending-topic-header">
                ...
                <span class="pending-status ${topic.status}">${topic.status}</span>
            </div>
            <div class="pending-topic-meta">
                <span class="pending-category">${topic.category}</span>
                <span class="pending-date">${new Date(topic.createdAt).toLocaleDateString()}</span>
            </div>
        `;
...
function approveTopic(topicId) {
    ...
    if (topicIndex !== -1) {
        ...
        const approvedTopics = JSON.parse(localStorage.getItem('approvedTopics') || '[]');
...
document.addEventListener('click', function(e) {
    if (e.target.closest('.pending-topic-item')) {
        ...
        const topicTitle = topicItem.querySelector('.pending-topic-title').textContent;

        // For demo purposes, show approval option
        if (confirm(`Approve topic: "${topicTitle}"? (Demo - In real app, only admins can do this)`)) {
